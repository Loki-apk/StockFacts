"""Fallback provider: Financial Modeling Prep — the ``/stable`` API.

Activates only when ``FMP_API_KEY`` is set. Used as the fallback when yfinance
errors, and (always, when keyed) as the peer resolver. Raises ``ProviderError``
on any failure so ``FallbackProvider`` can react.

Endpoints below are the ones reachable on the free tier; ``insider_transactions``
and ``short_interest`` are not, and return empty.
"""
...
from stocks.providers.base import ProviderError
