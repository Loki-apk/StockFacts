"""Persist each evaluation and diff it against the previous run for the ticker.

The diff is deterministic and is the single highest-value recurring-use feature:
surface only what changed since last time.
"""
...
def _db_path() -> Path:
    # Resolved per call, not at import time — STOCKS_RUNS_DB must be settable
    # after this module is first imported (tests isolate the DB this way).
    return Path(os.environ.get("STOCKS_RUNS_DB", "cache/runs.db"))
