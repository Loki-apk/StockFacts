"""-> ValuationContext. Where today's P/E sits vs. the stock's own history.

"P/E 24" means nothing; "P/E 24, 78th percentile of its own 5y range, peer
median 21" is a fact you can reason about.

Data reality: yfinance gives ~5 annual + ~5 quarterly income-statement periods
for free — not a daily fundamentals history. So we build a TTM-EPS curve by
linearly interpolating between the annual Diluted EPS points, apply it to the
daily close history, and take the distribution of the resulting P/E series.
Coarser than a paid fundamentals feed; honest about it in `notes`.
"""
...
from stocks.cache import cached, get_cached_or_fetch
from stocks.fetch.technicals import _history_payload
from stocks.models.reports import ValuationContext
