"""The last check before anything gets shown to a person — no LLM involved here.

1. Every number that shows up in the narrative text has to trace back to a
   number the pipeline already produced — either a typed field, or text the
   deterministic layer wrote itself (a risk-flag description, a headline, a
   valuation note).
2. No advice or prediction language, anywhere, ever.
3. If either check fails, the run doesn't just quietly render anyway — see
   report/render.py for what happens instead.
"""
...
from stockfact.models.reports import NarrativeSections, StockEvaluationReport
