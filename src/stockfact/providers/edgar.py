"""Pulls SEC filings through ``edgartools``.

It's free and it's the actual source document, which is why it's used for
real risk factors (10-K Item 1A) and MD&A (Item 7) instead of just letting
the LLM guess what the risks might be. The SEC asks you to identify yourself
with a real name + email on every request — that comes from
``SEC_USER_AGENT``.
"""
...
_IDENTITY = os.environ.get("SEC_USER_AGENT", "stockfact-eval you@example.com")
