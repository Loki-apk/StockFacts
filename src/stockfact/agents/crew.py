"""The three-agent CrewAI layer, step 3 in ARCHITECTURE.md.

These agents read the typed sub-reports already sitting on the RunContext
and write back NewsSentimentReport / RiskFlagReport / NarrativeSections.
They never touch the actual numbers, just describe them in words.

If CrewAI isn't installed, no LLM key is set, or something in this step
blows up, the run doesn't die — ``_fallback_reports`` below hands back a
plainer, LLM-free version of the same report.
"""
...
from stockfact.models.enums import Sentiment, Theme
...
_MAX_TOKENS = int(os.environ.get("STOCKFACT_LLM_MAX_TOKENS", "8000"))


def _verbose() -> bool:
    """Stream each agent's thinking + tool calls. `-v` on the CLI sets this."""
    return os.environ.get("STOCKFACT_CREW_VERBOSE") == "1"
...
def _no_think(model: str) -> bool:
    ...
    STOCKFACT_LLM_NO_THINK=0/1."""
    env = os.environ.get("STOCKFACT_LLM_NO_THINK")
    if env is not None:
        return env == "1"
...
def _build_agents():
    ...
    news = Agent(
        ...
        goal=(
            ...
            "strictly in the headline. Never drop a URL. Never invent items."
        ),
        backstory=(
...
def _passthrough_news(ctx: RunContext) -> NewsSentimentReport:
    from stockfact.fetch.news import NEWS_WINDOW_DAYS
...
def run_llm_layer(ctx: RunContext, *, allow_fallback: bool = True) -> RunContext:
    if os.environ.get("STOCKFACT_SKIP_LLM") or not _has_llm_key():
        if not os.environ.get("STOCKFACT_SKIP_LLM"):
