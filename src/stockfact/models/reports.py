"""The Pydantic models that keep this whole project honest.

Every fetch module and every agent returns one of these. Numbers only ever
live in typed fields that came from an actual data source — the only place
free-form text is allowed is NarrativeSections, and the QA pass checks that
text against everything else in here before it's allowed to render.
"""
...
class RelatedExposure(BaseModel):
    ...
    price_correlation_2y: float | None = None
