"""Plain-language version of the report — same numbers, explained like you're
twelve years old.

Fully deterministic, same as everything else that touches numbers here.
Every line is just a typed field restated in plain words, sometimes with a
lemonade-stand-style analogy if that makes it click faster. It never adds an
opinion and never says whether to buy — same rule as the rest of the system.
"""
    ...
    if not exposures:
        ...
        for exp in exposures:
            ...
            if exp.price_correlation_2y is not None:
                ...
                direction = (
                    "the same direction" if exp.price_correlation_2y >= 0 else "opposite directions"
                )
                line += (
